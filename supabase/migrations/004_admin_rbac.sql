-- Enforce admin-only access using explicit allowlist table.
...
-- Keep direct table access minimal: authenticated users can only see their own admin flag.
...
-- Replace broad click read policy with admin-only reads.
drop policy if exists "Authenticated can read click_events" on public.click_events;
drop policy if exists "Admins can read click_events" on public.click_events;
