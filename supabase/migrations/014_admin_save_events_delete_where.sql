-- pg-safeupdate (common on Supabase) rejects DELETE without WHERE.
-- Clearing all events used `DELETE FROM public.events`, which broke saves after removing every event.
